...
<?php
      ...
      { ?><div class="show">
         ...
         <?php $sql="SELECT a.question,r.answer FROM ask a JOIN reply r ON (a.id=r.qid) order by a.id";
          $res=mysqli_query($conn,$sql);
          if(mysqli_num_rows($res)>0){
              while($row=mysqli_fetch_assoc($res))
            {?>
             <p class="para"> Question-->>> <?php  echo $row['question'];?>
             
           <br><br> Answer-->> <?php    echo $row['answer']; ?></p>
